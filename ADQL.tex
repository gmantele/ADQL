...
% Redefine description blocks so that the definition is always on a new line instead of being just after the label:
\usepackage{enumitem} \SetLabelAlign{parleft}{\parbox[t]\textwidth{#1}}
\setdescription{ align=parleft, style=nextline }
...
\previousversion[https://www.ivoa.net/Documents/ADQL/20231215/]{ADQL-2.1}
...
\begin{description}
    ...
    \item[\texttt{power(x,y)}]
    \texttt{double power(double x, double y)} \\
    Returns the value of \textit{x} raised to the power of \textit{y}.
    
    \item[\texttt{radians(x)}]
    \texttt{double radians(double x)} \\
    Converts the angle \textit{x} from degrees to radians.
    
    \item[\texttt{sqrt(x)}]
    \texttt{double sqrt(double x)} \\
    Returns the positive square root of \textit{x}.
    
    \item[\texttt{rand(x)}]
    \texttt{double rand(double x)} \\
    Returns a random value between 0.0 and 1.0.
    The optional argument, \textit{x}, originally intended to provide a
    random seed, has undefined semantics. Query writers are advised to omit
    this argument.
    
    \item[\texttt{round(x,n)}]
    \texttt{double round(double x, integer n)} \\
    Rounds \textit{x} to \textit{n} decimal places.
    The integer \textit{n} is optional and defaults to 0 if not specified.
    A negative value of \textit{n} will round to the left of the decimal
    point.
    
    \item[\texttt{truncate(x, n)}]
    \texttt{double truncate(double x, integer n)} \\
...
\begin{description}
    \item[\texttt{acos(x)}]
    \texttt{double acos(double x)} \\
    Returns the arc cosine of \textit{x}, in the range of 0 through
    \(\pi\) radians. The absolute value of \textit{x} must be less than or
    equal to 1.0.
    
    \item[\texttt{asin(x)}]
    \texttt{double asin(double x)} \\
    Returns the arc sine of \textit{x}, in the range of -\(\pi\)/2 through
    \(\pi\)/2 radians. The absolute value of \textit{x} must be less than or
    equal to 1.0.
    
    \item[\texttt{atan(x)}]
    \texttt{double atan(double x)} \\
    Returns the arc tangent of \textit{x} , in the range of -\(\pi\)/2
    through \(\pi\)/2 radians.
    
    \item[\texttt{atan2(y,x)}]
    \texttt{double atan2(double y, double x)} \\
    Converts rectangular coordinates \textit{x,y} to polar angle. It
    computes the arc tangent of \textit{y/x} in the range of –\(\pi\)
    through \(\pi\) radians.
    
    \item[\texttt{cos(x)}]
    \texttt{double cos(double x)} \\
    Returns the cosine of the angle \textit{x} in radians, in the range of
    -1.0 through 1.0.
    
    \item[\texttt{sin(x)}]
    \texttt{double sin(double x)} \\
    Returns the sine of the angle \textit{x} in radians, in the range of
    -1.0 through 1.0.
    
    \item[\texttt{tan(x)}]
    \texttt{double tan(double x)} \\
...
This BNF is available in two equivalent forms:
\begin{itemize}
    \item A text form: \auxiliaryurl{adql-bnf.txt}
    \item A cross-referenced HTML form: \auxiliaryurl{adql-bnf.html}
\end{itemize}
}}
\label{sec:grammar}

\lstinputlisting[numbers=none,basicstyle=\ttfamily\small]{bnf/adql-bnf.txt}
...
\subsection{Between 2.1 and 2.2}
\label{sec:changes-2.2}

\begin{itemize}
    \item \textbf{Applied ADQL-2.1's errata}:
        \begin{itemize}
            \item Erratum 1 - Addition of auxiliary files for the BNF grammar
        \end{itemize}
    \item \textbf{General}
        \begin{itemize}
            \item \textbf{Updated}
                \begin{itemize}
                    \item Convert the tables for mathematical and trigonometrical
                          functions into lists (see \SectionRef{sec:math.functions})
                \end{itemize}
        \end{itemize}
\end{itemize}
